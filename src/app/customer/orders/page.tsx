                ...
                {filteredOrders.reverse().map((order) => (
                  <Card key={order.id} className="p-4 sm:p-6 hover:shadow-lg transition-shadow">
                    ...
                    {(order.rejectionReason || order.cancellationReason) && (
                      <div className={`mb-4 p-3 rounded-lg ${order.rejectionReason ? 'bg-red-50' : 'bg-orange-50'
                        }`}>
                        <p className={`text-sm font-medium mb-1 ${order.rejectionReason ? 'text-red-800' : 'text-orange-800'
                          }`}>
                          {order.rejectionReason ? 'Rejection Reason:' : 'Cancellation Reason:'}
                        </p>
                        <p className={`text-sm ${order.rejectionReason ? 'text-red-700' : 'text-orange-700'
                          }`}>
                          {order.rejectionReason || order.cancellationReason}
                        </p>
                      </div>
