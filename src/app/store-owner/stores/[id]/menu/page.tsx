...
import { useStoreOwnerMenu, useCreateMenuItem, useUpdateMenuItem, useDeleteMenuItem, useToggleMenuItemAvailability } from '@/hooks/api';
...
export default function MenuManagementPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const { data, isLoading } = useStoreOwnerMenu(params.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.error?.message || 'Failed to save menu item');
    }
  };
  ...
  const handleDelete = async (itemId: string) => {
    try {
      ...
      toast.error(error?.response?.data?.error?.message || 'Failed to delete menu item');
    }
  };
  ...
  const handleToggleAvailability = async (itemId: string) => {
    try {
      ...
      toast.error(error?.response?.data?.error?.message || 'Failed to toggle availability');
    }
  };
    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href={`/store-owner/stores/${params.id}`}>
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage your store menu items</p>
          </div>
        </div>
